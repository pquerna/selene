...
sln_log_level_e sln_log_level_get(selene_t *s) { return s->log_level; }
 ...
 */
static void sln_log_publish(selene_t *s, sln_log_level_e level, const char *str,
                            size_t len) {
  s->log_msg = str;
  s->log_msg_len = len;
  ...
  /* These are allocated off the stack, we don't want soemone to grab them later
   * out of place. */
  s->log_msg = NULL;
  s->log_msg_len = 0;
...
void selene_log_msg_get(selene_t *s, const char **log_msg,
                        size_t *log_msg_len) {
  *log_msg = s->log_msg;
