                           ...
                           sln_cryptor_t **p_enc)
{
  ...
  case SLN_CIPHER_AES_256_CBC:
    /* TODO: it is not clear from the docs why this is named AES-128, but if you
     * pass in a key size that is for AES-256, it works (?????) as if it was in AES-256 mode (!!!!!)
     */
    alg = kCCAlgorithmAES128;
  ...
  if (rv != kCCSuccess) {
    return selene_error_createf(SELENE_EIO, "CCCryptorCreate failed CCCryptorStatus=%d", rv);
  }
  else {
...
void
sln_cryptor_osx_cc_encrypt(sln_cryptor_t *enc, const void *data, size_t len, char *buf, size_t *blen)
{
  /* TODO: output buffer must be exactly the right size, our interface never pads for you */
...
void
sln_cryptor_osx_cc_destroy(sln_cryptor_t *enc)
{
  selene_t *s = enc->s;
  CCCryptorRef cryptor = enc->baton;
